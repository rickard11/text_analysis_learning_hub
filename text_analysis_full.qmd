...
stringr::str_to_title(x) #capitalize only first letter of each word
str_to_lower(x) #make everything lowercase
...
str_replace(x, ' ', '_')
str_replace_all(x, ' ', '_') #take a pattern and replace with a different pattern
...
coral_depth <- coral_depth %>%
  ...
  mutate(depth_min = min(depth_num),
         n = n()) %>%
